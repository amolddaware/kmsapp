 ...
 * @author Fluidscapes
 */
public class AddGRAction extends org.apache.struts.action.Action {
...
//                            smbFilepath = smbUrl + File.separator + newFilename.toLowerCase();
...
//                            System.out.println(subject);
                        ...
                        if (item.getFieldName().equals("deptmarathi")) {
                            deptmarathi = item.getString("UTF-8").trim();
//                            System.out.println(subject);
                        }
...
//                }
                ...
                String insertData = "insert into tbl_news_dataentry(userid,sourcedate,dept,deptmarathi,"
                        + "subject,titlemarathi,grcode,docpath,datetime,ipaddress)"
                        + " values(?,?,?,?,?,?,?,?,?,?)";
                PreparedStatement pstmtData = dh.prepareStatement(insertData);
                pstmtData.setString(1, userid);
                pstmtData.setString(2, date);
                pstmtData.setString(3, dept);
                pstmtData.setString(4, deptmarathi);
                pstmtData.setString(5, title);
                pstmtData.setString(6, titlemarathi);
                pstmtData.setString(7, grcode);
                pstmtData.setString(8, fileDocPath);
                pstmtData.setString(9, date11);
                ...
                if (pstmtData.executeUpdate() > 0) {
                    ...
                    if (rs.next()) {
                        ...
                        if (pstmtup.executeUpdate() > 0) {
                            ...
                            response.sendRedirect("gr_add.jsp?workcode=" + workcode + "&success");
                        } else {
